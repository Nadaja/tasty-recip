...
export default {
    ...
    state() {
        return {
            ...
            userLogin: {},
            isLogin: false,
            apiKey: "AIzaSyD17ImzQcIAvk8tsBjZ7cEKVkFVT97vei4"
        }
    },
    mutations: {
    ...
    actions: {
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                const { data } = await axios.post(
                    `https://vue-js-project-665be-default-rtdb.asia-southeast1.firebasedatabase.app/user.json?auth=${state.token}`, payload);
        ...
        async getLoginData({ commit, dispatch, state }, payload) {
            const APIkey = state.apiKey;
            const authUrl =
        ...
        async getUser({ commit }, payload) {
            try {
                const { data } = await axios.get(`https://vue-js-project-665be-default-rtdb.asia-southeast1.firebasedatabase.app/user.json`)
                for (let key in data) {
